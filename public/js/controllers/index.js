...
var app = angular.module("hawk", ["ng", "ngProgress", "ngRoute", "ngSanitize", "ngResource", "naif.base64", "ui.bootstrap", "bootstrap-tagsinput", "ui-rangeSlider", "sparklines", "autocomplete", "angularUtils.directives.dirPagination"]);
...
app.controller("interface", function($scope, $http, $route, $routeParams, $location){      
    $scope.$route = $route;
    $scope.$location = $location;
    $scope.$routeParams = $routeParams;

    $scope.loaddata = function(id, url){
        $http({
            method: "GET",
            url: url,
        }).then(function(res){
    ...
    $scope.loaddata("lng", "i18n/en.json");
});   
...
app.config(function($routeProvider, $locationProvider) {             
    ...
    $routeProvider.when('/filters-add', {
        templateUrl: 'filters-manager.html',
        controller: 'filters-manager'
    });

    $routeProvider.when('/filters-edit', {
        templateUrl: 'filters-manager.html',
        controller: 'filters-manager'
    });
    ...
    $routeProvider.when('/databases-add', {
        templateUrl: 'databases-add.html',
        controller: 'databases-add'
    });

    $routeProvider.when('/mappers', {
        templateUrl: 'mappers.html',
        controller: 'mappers'
    });

    $routeProvider.when('/mappers-add', {
        templateUrl: 'mappers-add.html',
        controller: 'mappers-add'
    });
