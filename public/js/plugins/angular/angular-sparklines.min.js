...
angular.module('sparklines').service('SparkUtils', function () {
  ...
  svc.areaChart = function (id, data, options) {
    ...
    var area = d3.svg.area()
      //.attr('style', "fill: "+options.fillColor+";")
      .x(function(d, i) { return x(i); })
      .y0(options.height)
      .y1(function(d) { return y(d); });
    // add the area fill to the graph
    ;
    graph.append('path').datum(data).attr('class', ((options.class) ? options.class : "area")).attr('d', area);
  };
  ...
  svc.barChart = function (id, data, options) {
      d3.select(id).html('');
    // create the d3 SVG element
    var graph = d3.select(id).append('svg:svg').attr('class', ((options.class) ? options.class : "bar")).attr('width', options.width).attr('height', options.height);
    // Y scale
  ...
  return {
    ...
    link: function (scope, element, attrs, ngModel) {
        ...
        var render = function(){
            ...
            //data = scope.test ? Sparklines.randomNumbers(20, 100) : ngModel || [];
            angular.isString(ngModel.$viewValue) ? model = ngModel.$viewValue.replace(/(^,)|(,$)/g, "") : model = ngModel.$viewValue;
                        
            if(model)
                angular.isArray(model.data) ? data = model.data : data = model.data.split(',');
            else
                data = [];
            
            //console.log(ngModel);
            
            options = model ? Sparklines.mergeOptions(Sparklines.defaultOptions, model) : Sparklines.defaultOptions;
